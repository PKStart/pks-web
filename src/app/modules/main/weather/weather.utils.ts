...
export function transformWeather(res: WeatherResponse): Weather {
  ...
  const daily: DailyWeather[] = res.daily.map(day => ({
    dayOfWeek: format(new Date(day.dt * 1000), 'EEEE'),
...
export function getPrecipitationLh<T extends { rain?: { '1h': number }; snow?: { '1h': number } }>(
  obj: T
): string | undefined {
  if (obj.rain)
...
export function getPrecipitation<T extends { rain?: number; snow?: number }>(
  obj: T
): string | undefined {
